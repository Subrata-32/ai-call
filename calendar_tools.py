    ...
    for attempt in range(2):  # retry once on transient error
        try:
            ...
            resp = _requests_session.get(
                ...
                timeout=4,  # fail fast — reduced from 8s
            )
            ...
            for s in raw_slots:
                ...
                try:
                    label = dt.strftime("%-I:%M %p")   # Linux/Mac
                except ValueError:
                    label = dt.strftime("%I:%M %p").lstrip("0")  # Windows fallback
                slots.append({"time": s["time"], "label": label})
            logger.info(f"[CAL] {len(slots)} slots for {date_str}")
            return slots
        except requests.Timeout:
            logger.warning(f"[CAL] Timeout fetching slots (attempt {attempt+1})")
            if attempt == 0:
...
async def _create_booking_calcom(
    ...
    try:
        async with httpx.AsyncClient(timeout=4.0) as client:  # fail fast — reduced from 8s
        ...
        return {"success": False, "booking_id": None, "message": "Booking timed out — please try again."}
    except Exception as e:
